...
NOTE: For example, extensions defined by the Modelica Association might make use of the `org.modelica.ssp` namespace.
...
The SSP standard uses semantic version numbers, as defined in <<SV200>>, where the standard version consists of a triple of version numbers, consisting of major version, minor version, and patch version numbers.

NOTE: Example: `1.2.3` for major version 1, minor version 2 and patch version 3.
...
Usually those references make use of relative URIs, where the base URI that those relative URIs are resolved against are specified in the SSP standard.

NOTE: E.g. for SSDs, relative URIs to other content are usually resolved against the URI of the SSD, but can optionally also be resolved against the URI of referenced components, e.g. in order to reference a file inside a referenced FMU.
