...
{empty}[ _Note that the value attribute is a signed integer value with 64-bit precision in SSP 2.0, to support the enhanced value space of enumerations in FMI 3.0, but it is a 32-bit signed integer value in SSP 1.0, to be compatible with the enumeration value space of FMI 2.0._
_The initial release of SSP 2.0 had erroneously retained the 32-bit signed integer value space for enumeration items, which is now corrected._ ]
...
This element provides for a linear transformation of the source value to the target value, i.e. in the calculation latexmath:[\text{target} = \text{factor} \cdot \text{source} + \text{offset}].
