...
For systems by default the placement of connectors for the inside and outside view of the system is identical, the special coordinate system is just translated to different actual coordinate systems, namely the one determined by the ElementGeometry for the outside view, and the one determined by SystemGeometry for the inside view.

For systems optionally a different placement of connectors for the inside view of the system can be specified with the systemInnerX and systemInnerY coordinates. This enables to preserve the inside system layout when integrating a system in a system structure and to avoid unintended changes to the position of the connectors on the inside view of the system when doing layout changes on the outside view and vice versa.
