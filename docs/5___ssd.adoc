...
{empty}[ _Note: The element name Clock is used for two different child elements in the Connector context. In the type choice, Clock refers to the SystemStructureCommon element `ssc:Clock` (the connector type). The additional Clock child element with attribute `name` refers to the SystemStructureDescription element `ssd:Clock` (the connector-to-clock association)._ ]
