== System Structure Description (SSD) [[SSD]]
...
image:images/SystemStructureDescription_SystemStructureDescription.svg[]
...
|version
|This required attribute specifies the version of this specification that the system description conforms to.
...
|System
|This required element specifies the root system of the model, see <<System>>.

|Enumerations
|This optional element must contain definitions for all enumerations referenced in the system description file.
See <<Enumerations>> for its definition.

|Units
|This optional element must contain definitions for all units referenced in the system description file.
...
image:images/SystemStructureDescription_DefaultExperiment.svg[]
...
NOTE: Note that in contrast to FMI 2.0/3.0 only start and stop time are specified here, since values like step size or tolerance depend on the specific solver or master algorithms employed and are hence not specified in this global element.
Additional solver or master algorithm specific information can be supplied through the annotation mechanism, or using a future layered standard.

NOTE: The handling of systems comprising components with differing units for the independent variable depends on the implementation.
It should be noted that since FMI 2.0 the unit of the independent variable for FMUs is clearly specified:
It defaults to seconds, however other units can be specified by explicitly defining the independent variable.
This standard does not specify additional measures to deal with differing independent variable units, but leaves this to the implementation.
...
image:images/SystemStructureDescription_TElement.svg[]
...
|name
|This required attribute gives the model element a name, which is used to identify the model element inside its parent system.
...
|ParameterBindings
|This optional element specifies the set of parameter bindings of this model element.
See below for details.
|===

==== Connectors

image:images/SystemStructureDescription_Connectors.svg[]
...
NOTE: Note that connectors do not have to be referenced in connections.
...
The same is true for variables/ports that are not referenced through a connector at all.

image:images/SystemStructureDescription_Connector.svg[]
...
|Real / Float64 / Float32 / Integer / Int8 / UInt8 / Int16 / UInt16 / Int32 / UInt32 / Int64 / UInt64 / Boolean / String / Enumeration / Binary / Clock
|Exactly one of these elements can be present to specify the type of the Connector.
See <<ssc_types>> for details.

|Dimension
|One or more of these optional elements specify the connector array dimensions, making the connector an array connector.
...
|ConnectorGeometry
|This optional element defines the geometry information of the connector.
See below for details.
|===
...
NOTE: Note that the current opaque mapping of more advanced types to Binary connectors is a temporary solution, and a more detailed mapping may be provided in future versions of the standard supporting more complex data types.

===== ConnectorGeometry

image:images/SystemStructureDescription_ConnectorGeometry.svg[]
...
|rotation|Optional attribute defines an additional rotation in degrees, where positive numbers indicate left rotation (x→y).
...
|systemInnerRotation|Optional attribute giving a separate rotation in degrees of the connector for the inside view of a system, where positive numbers indicate left rotation (x→y).
|===

[NOTE]
====
.Graphical example for a ConnectorGeometry
image:images/Example_ConnectorGeometry.svg[width=500]
====

==== ElementGeometry

image:images/SystemStructureDescription_ElementGeometry.svg[]
...
NOTE: Sometimes such a counter clockwise orientation is also called a left rotation (x->y), with the coordinate system orientation: x -> right, y -> up)
...
NOTE: If no explicit icon is given, the icon used by the tool to represent the system should be rotated accordingly.
...
|iconSource
|Optional attribute defines an icon URI with the same semantics as for the source attribute of the Component element.
...
image:images/Example_ElementGeometry.svg[width=600]
====
...
image:images/Example_Icon0.svg[width=300]
...
image:images/Example_Icon1.svg[width=600]
...
image:images/Example_Icon2.svg[width=300]
...
image:images/Example_Icon3.svg[width=600]
...
image:images/Example_Icon4.svg[width=300]
...
image:images/Example_Icon5.svg[width=600]
...
image:images/Example_Icon6.svg[width=300]

*Example 7: Horizontal flip*
...
image:images/Example_Icon7.svg[width=600]

*Example 8: Horizontal flip*
...
image:images/Example_Icon8.svg[width=300]

*Example 9: Horizontal flip*
...
image:images/Example_Icon9.svg[width=300]

*Example 10: Vertical flip*
...
image:images/Example_Icon10.svg[width=600]

*Example 11: Horizontal and vertical flip*
...
image:images/Example_Icon11.svg[width=600]

*Example 12: Horizontal and vertical flip*
...
image:images/Example_Icon12.svg[width=300]

*Example 13: Horizontal and vertical flip*
...
image:images/Example_Icon13.svg[width=600]

*Example 14: Horizontal and vertical flip*
...
image:images/Example_Icon14.svg[width=300]

*Example 15: Horizontal and vertical flip*
...
image:images/Example_Icon15.svg[width=300]
====

==== ParameterBindings

image:images/SystemStructureDescription_ParameterBindings.svg[]
...
The hierarchical name of those parameters inside system B are SP1 and C.P2 respectively, and the hierarchical name of P2 inside element C is just P2.
...
NOTE: For example, for a system A containing a system B with component C and variable D, and system A also containing a component called B.C and variable D, both variables will have the hierarchical name A.B.C.D.
...
|type
|Optional attribute giving the MIME type of the parameter source, which defaults to application/x-ssp-parameter-set to indicate the SSP parameter set file format.
...
|prefix
|Defines the optional prefix for name resolution and mapping purposes for this binding.
...
|ParameterMapping
|This optional element provides an optional parameter mapping, which specifies how the parameter names and values provided in the parameter source are to be mapped to the parameters of the component or system in question.
...
image:images/SystemStructureDescription_ParameterValues.svg[]
...
image:images/SystemStructureDescription_ParameterMapping.svg[]
...
|type
|Optional attribute giving the MIME type of the parameter mapping, which defaults to `application/x-ssp-parameter-mapping` to indicate the SSP parameter mapping file format.
...
image:images/SystemStructureDescription_System.svg[]
...
image:images/SystemStructureDescription_Elements.svg[]
...
|Component
|A component, see <<Component>>.

|SignalDictionaryReference
|A reference to a signal dictionary, see <<SignalDictionaryReference>>.

|System
|A nested system, see <<System>>.
|===

==== Connections

image:images/SystemStructureDescription_Connections.svg[]

This optional element provides the connections between connectors of the system, connectors of its elements and inbetween those connectors.

image:images/SystemStructureDescription_Connection.svg[]
...
|startElement
|Optional attribute giving the the name of the element that contains the connector given as startConnector.
If the attribute is not present, then the startConnector names a connector on this system.

|startConnector
|Required attribute giving the name of the connector that is the start of the connection.
...
|startIndices
|Optional attribute giving the list of indices of an array connector that this connection is restricted to.
If not supplied this indicates that this connection applies to the whole connector, not just a single element.

|endElement
|Optional attribute giving the name of the element that contains the connector given as endConnector.
If the attribute is not present, then the endConnector names a connector on this system.

|endConnector
|Required attribute giving the name of the connector that is the end of the connection.
...
|Optional attribute specifying whether automatic conversions between start and end connector are performed using unit information potentially available for both start and end definitions.
...
|Specifies an optional transformation for the connection.
...
image:images/SystemStructureDescription_ConnectionGeometry.svg[]
...
image:images/Example_ConnectionGeometry.svg[width=600]
====
...
image:images/SystemStructureDescription_SignalDictionaries.svg[]

This optional element provides the set of defined signal dictionaries for the system.

image:images/SystemStructureDescription_SignalDictionary.svg[]
...
|name
|Required attribute giving the signal dictionary a name, which shall be unique within the directly enclosing system.
...
|type
|Optional attribute giving the MIME type of the signal dictionary, which defaults to `application/x-ssp-signal-dictionary` to indicate the SSP signal dictionary file format.
No further types are currently defined, but can of course be added at a later date.
...
image:images/SystemStructureDescription_SystemGeometry.svg[]
...
NOTE: The visual appearance of a length of 1 should be (roughly) 1 mm.
...
image:images/Example_NestedElementGeometry.svg[width=800]
...
image:images/SystemStructureDescription_GraphicalElements.svg[]
...
image:images/SystemStructureDescription_Note.svg[]
...
image:images/SystemStructureDescription_Component.svg[]
...
|type
|Optional attribute giving the MIME type of the component, which defaults to `application/x-fmu-sharedlibrary` to indicate the type of the component.
...
NOTE: Example: `type="text/x-modelica; path=Modelica.Mechanics.Rotational.Interface.Flange_A"`.
...
NOTE: For example for an FMU called MyDemoFMU.fmu, that is located in the resources directory of an SSP, the correct URI would be resources/MyDemoFMU.fmu.
...
|implementation
|When the referenced component is an FMU that contains multiple implementations, this optional attribute can be used to determine which FMU implementation should be employed.

NOTE: For example Co-Simulation and Model Exchange.
...
image:images/SystemStructureDescription_SignalDictionaryReference.svg[]

A signal dictionary reference is an element of a system that references a signal dictionary, i.e. it is the placeholder of the signal dictionary that allows access to the contents of the signal dictionary through its connectors, which must reference signal dictionary entries.
