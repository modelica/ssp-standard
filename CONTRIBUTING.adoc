...
The SSP Specification is written in http://asciidoc.org/[AsciiDoc], a plain-text mark-up format that can be converted to HTML, PDF and https://asciidoctor.org/docs/convert-documents/#selecting-an-output-format[other formats].

If you haven't already done so https://help.github.com/articles/cloning-a-repository/[clone] the repository with

  git clone https://github.com/modelica/ssp-standard.git
...
+
  asciidoctor -B <ssp-standard> -b html5 --failure-level WARN -v docs/index.adoc
+
where `<ssp-standard>` is the path to the cloned repository
...
. https://help.github.com/articles/fork-a-repo/[Fork] the project, clone your fork, and configure the remotes:
  ...
  git clone https://github.com/<your-username>/ssp-standard
  # Navigate to the newly cloned directory
  cd ssp-standard
  # Assign the original repo to a remote called "upstream"
  git remote add upstream https://github.com/modelica/ssp-standard
...
Releases and pre-releases are tagged on the respective branches following the https://semver.org/[Semantic Versioning] when the XSD schemas or file structure of the SSP archive change.
+
...
* update the SSP version in the schemas, and XML examples
* update <<CHANGELOG.adoc#,CHANGELOG.adoc>>
* download the `ssp-standard.zip` build artefact and check for completeness
* create a tag on `main` (e.g. `v3.0-rc.1`) and push it together with a commit to trigger the CI
* create a new https://github.com/modelica/ssp-standard/releases[release] on GitHub for the tag
** check `This a pre-release` if it's not the final release
** add the text from `CHANGELOG.adoc`
** add the version to `ssp-standard.zip` (e.g. `ssp-standard-3.0-rc.1.zip`) and upload it to the release

== Contributor License Agreement

All contributors have to sign the https://github.com/modelica/ModelicaAssociationCLA/blob/master/ModelicaAssociationCLA.md[Modelica Association Contributor License Agreement (CLA)].
Therefore, the first step is getting your company to agree and sign the CLA.
The CLA ensures that all IP contributed to the SSP standard will be licensed to the Modelica Association (MA) which in turn will sublicense the SSP standard to tool vendors implementing it and end users using it, free of charge.
...
----
== SSP Common Concepts [[ssp-common-concepts]]

// ...

These parts are defined in <<ssp-common-concepts>>.
----
...
Non-normative text should be formatted using "NOTE" admonitions.
Examples:
